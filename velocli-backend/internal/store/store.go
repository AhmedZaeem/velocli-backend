...
func (s *Store) UpsertTemplateZip(t MainTemplate, encryptedBlob []byte) error {
	if t.ID == "" {
		t.ID = randID("tpl")
	}
	if t.BlobID == "" {
		t.BlobID = randID("blob")
	}

	if err := os.MkdirAll(s.blobDir, 0o755); err != nil {
		return err
	}
	blobPath := filepath.Join(s.blobDir, t.BlobID+".bin")
	if err := writeFileAtomic(blobPath, encryptedBlob, 0o600); err != nil {
		return err
	}
	...
	found := false
	for i := range s.catalog.MainTemplates {
		if s.catalog.MainTemplates[i].ID == t.ID {
			if t.Deps == nil && s.catalog.MainTemplates[i].Deps != nil {
				t.Deps = s.catalog.MainTemplates[i].Deps
			}
			if t.Content == "" {
				t.Content = s.catalog.MainTemplates[i].Content
			}
			s.catalog.MainTemplates[i] = t
			found = true
			break
		}
	}
	if !found {
		s.catalog.MainTemplates = append(s.catalog.MainTemplates, t)
	}
	if err := s.saveLocked(); err != nil {
		return err
	}
...
func (s *Store) GetEncryptedTemplateBlob(templateID string) ([]byte, MainTemplate, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	for _, t := range s.catalog.MainTemplates {
		if t.ID != templateID {
			continue
		}
		if t.BlobID == "" {
			return nil, MainTemplate{}, errors.New("missing blob")
		}
		blobPath := filepath.Join(s.blobDir, t.BlobID+".bin")
		data, err := os.ReadFile(blobPath)
		if err != nil {
			return nil, MainTemplate{}, err
		}
		return data, t, nil
	}
